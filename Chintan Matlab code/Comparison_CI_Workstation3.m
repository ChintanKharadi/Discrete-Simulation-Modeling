...
c1=zeros(300,1);
 for i=1:1:300
     c1(1,i)=abs(workstation3(i,1)*choose_value(i,1));
 end



%%  Normal Distribution Generation based upon random numbers
m1 = mean(workstation3); %mean of the Random Inspection time (i.e. data)
Min1 = min(workstation3);
Max1 = max(workstation3);
n1= m1/(Max1-Min1);
x = 0:.00001:.99999;
z1=normcdf(x);
...
time_avg1 = work3_time / (product3_count-1);
...
while (component1_count < Loop_limit) 
    if(time_component1<0)
        %Calling Inspector1 function to calculate inspection time and
        %assign components to respective buffer
        [bufferc1w1,bufferc1w2,bufferc1w3,time_component1,insp1_idle,component1_count]=inspector1_updated(bufferc1w1,bufferc1w2,bufferc1w3,component1,time_component1,insp1_idle,component1_count);
        insp1_time=insp1_time +1.00;
    elseif (time_component1 > 0)
        time_component1 = time_component1 - 1.00005999;
        insp1_time = insp1_time + 1.0000; 
...
time_avg2 = work3_time / (product3_count-1);
end
...
%% CI
pd_use = fitdist(component3,'Normal');
ci_Testing = paramci(pd_use,'Alpha',.01);
...
%% Comparison of Original and Alternative models using CRN (Common Random Numbers) meth
% time_avg1=0; %As it shows Nan and no products are produced in original model. 
Difference = time_avg1 - time_avg2;

std_avg=(Difference-mean(Difference))^2;

CI1 = mean(time_avg1)-mean(time_avg2) + (2.23*(std_avg/sqrt(n)));
CI2 = mean(time_avg1)-mean(time_avg2)  - (2.23*(std_avg/sqrt(n)));
ci = sprintf('CI (95 percentage)for 9 replications :\t[%d,%d]',CI1,CI2);
disp(ci);
...
diff = sprintf('Difference of mean in actual and Alternative model is %d',mean(Difference));
disp(diff);