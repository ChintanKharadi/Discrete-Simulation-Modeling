...
c2=zeros(300,1);
 for i=1:1:300
     c2(1,i)=abs(workstation2(i,1)*choose_value(i,1));
 end
...
time_avg(n) = work2_time / (product2_count-1);
...
%% CI
pd_use = fitdist(workstation2,'Normal');
ci_Testing = paramci(pd_use,'Alpha',.03);
...
ci = sprintf('CI (95 percentage)for 12 replications :\t[%d,%d]',mean_lower,mean_upper);
disp(ci);
