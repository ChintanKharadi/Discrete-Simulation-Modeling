...
c2=zeros(300,1);
 for i=1:1:300
     c2(1,i)=abs(workstation1(i,1)*choose_value(i,1));
 end
...
time_avg(n) = work3_time / (product3_count-1);
...
%% CI
pd_use = fitdist(workstation1,'Normal');
ci_Testing = paramci(pd_use,'Alpha',.05);
...
ci = sprintf('CI (95 percentage)for 9 replications :\t[%d,%d]',mean_lower,mean_upper);
disp(ci);
